...
const extractTextPluginOptions = shouldUseRelativeAssetPaths
                                 ? // Making sure that the publicPath goes back
                                   // to to build folder.
                                 {
                                   publicPath : Array(
                                     cssFilename.split('/').length).join('../'),
                                 }
                                 : {};
  ...
  plugins : [
    ...
    new webpack.optimize.UglifyJsPlugin({
                                          compress  : {
                                            ...
                                            // with Uglify breaking seemingly
                                            // valid code:
                                          ...
                                          output    : {
                                            comments   : false,
                                            // Turned on because emoji and
                                            // regex is not minified properly
                                            // using default
    ...
    new SWPrecacheWebpackPlugin({
                                  ...
                                  // parameter is appended to requests used to
                                  // populate the caches, to ensure the
                                  // responses are fresh. If a URL is already
                                  // hashed by Webpack, then there is no
                                  // concern about it being stale, and the
                                  // cache-busting can be skipped.
                                  dontCacheBustUrlsMatching     : /\.\w{8}\./,
                                  filename                      : 'service-worker.js',
                                  logger (message) {
                                    if (message.indexOf(
                                      'Total precache size is') === 0)
                                    {
                                      // This message occurs for every build
                                      // and is a bit too noisy.
                                      return;
                                    }
                                    if (message.indexOf(
                                      'Skipping static resource') === 0)
                                    {
                                      // This message obscures real errors so
                                      // we ignore it.
                                  ...
                                  minify                        : true,
                                  // For unknown URLs, fallback to the index
                                  // page
                                  navigateFallback              : publicUrl
                                                                  + '/index.html',
                                  // Ignores URLs starting from /__ (useful for
                                  // Firebase):
                                  // https://github.com/facebookincubator/create-react-app/issues/2237#issuecomment-302693219
                                  navigateFallbackWhitelist     : [/^(?!\/__).*/],
                                  // Don't precache sourcemaps (they're large)
                                  // and build asset manifest:
